...
description: Update the github.com/xgo-dev/llgo dependency version across the llcppg project — Go module, CI workflows, and setup action. Use this skill whenever the user asks to bump, update, or upgrade the llgo version, or mentions changing the llgo dependency, even if they don't use the exact phrase "update llgo version".
---

# Update LLGo Version

Update `github.com/xgo-dev/llgo` to a new version across the entire project. Six files reference the llgo version and they all need to stay in sync.
...
Read `go.mod` and note the current `github.com/xgo-dev/llgo` version — you'll need it for the commit message.
...
```bash
go get github.com/xgo-dev/llgo@<target_version>
go mod tidy
```
...
```
build(deps): bump github.com/xgo-dev/llgo from <old_version> to <new_version>
```

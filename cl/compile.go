...
type Config struct {
	...
	// Deps specifies the dependencies (optional). They are Go packages that generated by
	// llcppg for dependency C/C++ libraries, include C/C++ standard library.
	Deps []string
...
func NewPackage(pkgPath, pkgName string, files []Source, conf *Config) (ret Package, err error) {
	...
	for _, dep := range conf.Deps {
		ctx.importPkg(dep) // TODO(xsw): optimize
	}
